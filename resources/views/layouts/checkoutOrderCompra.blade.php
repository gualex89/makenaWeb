...
<body class="home_motorcycle">
    ...
    <div class="header_content_wrap d-flex align-items-center" id="cintillo_carrito">
      <div class="container-fluid prl_90">
        <div class="row align-items-center">
          ...
          <div class="col-lg-8">
            <h2 class="header_title text-center text-white">Orden de Compra</h2>
          </div>
          <div class="col-lg-2">
    ...
    <div class="sidebar-menu-wrapper">
      <div class="sidebar_mobile_menu">
        ...
        <div class="msb_widget brand_logo text-center">
          <a href="/">
            <img src="images/logo/logo_black.png" srcset="images/logo/logo_black.png"
              alt="logo negro makena">
          </a>
        </div>
          ...
          <div class="accordion" id="accordionExample">
            ...
            <div class="card">
              <div class="card-header" id="headingOne">
                <h4 class="mb-0">
                  <button id="primerAcordeon" type="button" data-toggle="collapse"
                    ...
                    <div class="row">
                      ...
                      <div class="col-lg-6 form_item">
                        ...
                        <input type="tel" name="telefono" class="form-control"
                          id="telefonoInput" pattern="\d*" title="Ingrese solo números"
                          value="{{ old('telefono') }}">
                      </div>
                    </div>
                    <button id="continuarButton1"
                      class="custom_btn bg_carparts_red text-uppercase special_button"
                      style=" max-width: 200px;">Continuar</button>
                  </div>
                ...
                <div class="card-body">
                  <div class="form_wrap">
                    <div class="row">
                      <div class="col-lg-6">
                        <div class="pl-2">
                          <input type="radio" id="radioRetiro" name="tipoEntrega"
                            value="retiro" class="form-check-input">
                          <label for="retiro">Retiro</label>
                        </div>
                      </div>
                      <div class="col-lg-6">
                        <div class="pl-2">
                          <input type="radio" id="radioEnvio" name="tipoEntrega"
                            value="envio" class="form-check-input">
                          <label for="envio">Envío</label>
                        </div>
                      </div>
                    </div>

                    <div id="mensajeEntrega" style="display: inline-block" class="mt-3"></div>
                    <small class="pl-5" id="soloNumeros" style="display: none;">Solo
                      Números</small>
                  ...
                  <button id="continuarButtonRetiroSucursal"
                    class="custom_btn bg_carparts_red text-uppercase special_button"
                    style=" max-width: 200px; display:none">Continuar</button>
                    ...
                    <div class="col-lg-4 col-md-12 col-sm-12 col-xs-12">
                      ...
                      <div class="cart_pricing_table pt-0 text-uppercase"
                        ...
                        <ul class="ul_li_block clearfix">
                          ...
                          <li id="descuentoSpan" style="display: none">
                            <span>Descuento</span> <span id="descuentoValor">$0</span>
                          </li>
                          ...
                          <div>
                            ...
                            <button id="pay-with-transfer"
                              class="custom_btn bg_transferencia mt-3">Pagar con Transferencia</button>
                            <ul class="ul_li_block clearfix">
                              <div>
                            ...
                            <button id="apply-discount"
                              class="custom_btn bg_success">Aplicar Descuento</button>
                          </div>
  ...
  <script>
    ...
    $(document).ready(function() {
      ...
      const orden = @json($orden);

      function updatePrices(subtotal, shippingCost, total) {
        document.getElementById('subtotalValor').textContent = `$${subtotal.toFixed(2)}`;
        document.getElementById('envioValor').textContent = `$${shippingCost.toFixed(2)}`;
        document.getElementById('totalValor').textContent = `$${total.toFixed(2)}`;
      }

      function updatePricesEnvio(subtotal, shippingCost, total) {
        newTotal = total + shippingCost
        document.getElementById('subtotalValor').textContent = `$${subtotal.toFixed(2)}`;
        document.getElementById('envioValor').textContent = `$${shippingCost.toFixed(2)}`;
        document.getElementById('totalValor').textContent = `$${newTotal.toFixed(2)}`;
      }

      function updateCartItems() {
        const cartTableBody = $('.cart_section table tbody');
        cartTableBody.html('');
        console.log("aqui la orden", orden);
        subtotal = 0;
        ...
        <td>
          <div class="cart_product">
            ...
            <div class="item_content">
              <h4 class="item_title">${item.diseno}</h4>
              <span class="item_marca">${item.marca}</span>
              <span class="item_type">${item.modelo}</span>
              <span class="item_nombre_imagen">${item.tipoFunda}</span>
            </div>
          </div>
        </td>
    ...
    `;
        ...
        console.log("aqui la sub", subtotal);

        let shippingCost = parseFloat($('input[name="carrier"]:checked').val()) || 0;
        console.log("aqui la shipping", shippingCost);
        ...
        console.log("aqui la total", total);
        updatePricesEnvio(subtotal, shippingCost, total);
      }
      ...
      $('#continuarButtonRetiro').on('click', function() {
        ...
        if ($('#radioRetiro').is(':checked')) {
          ...
          shippingCost = 0;
          updatePricesEnvio(subtotal, shippingCost, total);
          saveOrder(total, subtotal, shippingCost)
            .then(idOrder => {
      ...
      $('#continuarButton2').on('click', function() {
        ...
        if (calle !== '' && altura !== '') {
          ...
          shippingCost = parseFloat($('input[name="carrier"]:checked').val()) || 0;
          updatePricesEnvio(subtotal, shippingCost, total);
          saveOrder(total, subtotal, shippingCost)
            .then(idOrder => {
              console.log(total, subtotal, shippingCost, idOrder, 0);
              return mercadoPago(total, subtotal, shippingCost, idOrder, 0).then(
                preference_id => {
                  return {
                    idOrder,
    ...
    function showPickupPoints(result) {
      ...
      puntosDeEntrega.forEach((puntoDeEntrega, index) => {
        ...
        const DireccionPunto = puntoDeEntrega.location.street + " " + puntoDeEntrega.location
          .street_number + " - " + puntoDeEntrega.location.city + ", " + puntoDeEntrega.location.state;
    ...
    function saveOrder(total, subtotal, shippingCost) {
      return new Promise((resolve, reject) => {
