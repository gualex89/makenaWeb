...
@section('content')
    <div id="create" class="tabcontent active mt-5 contenedor-tucuadro">
        <div class="wrapper-content-create">
            <div class="heading-section">
                ...
                <p class=" mt-3 text-align-start">
                    Nuestros cuadros están fabricados en <strong class="resaltado-color">aluminio</strong> con un grosor de
                    <strong class="resaltado-color">1 mm</strong>,
                    con acabado <strong class="resaltado-color">brillante</strong> tipo espejo e <strong
                        class="resaltado-color">impresión 4K</strong>
                        ...
                        <div class="wrap-upload w-full">
                            <form id="uploadForm">
                                <label class="uploadfile" id="uploadLabel"
                                    style="cursor:pointer; display:flex; align-items:center; justify-content:center; gap:30px; flex-wrap:wrap;">
                                    ...
                                    <div id="uploadText" style="text-align:center; flex:1; min-width:250px;">
                                        ...
                                        <input type="file" name="file" id="fileInput" accept="image/png, image/jpeg "
                                            style="display:none;">
                                    </div>
                                    ...
                                    <div id="previewContainer" style="display:none; flex:1; text-align:center;">
                                        <img id="preview" src="" alt="Vista previa"
                                            style="width:auto; height:150px; border-radius:8px; object-fit:cover; box-shadow:0 0 8px rgba(0,0,0,0.2);">
                                    </div>
            ...
            <div class="opciones-container mt-5">
                ...
                <div class="grupo text-align-center">
                    ...
                    <div class="botones mt-3 d-flex flex-wrap" style="gap: 4rem;">
                        @foreach ($cuadrosActivos as $cuadro)
                            <div class="row-mb-3">
                                <button type="button" class="btn-opcion" data-group="tamaño"
                                    data-value="{{ $cuadro->name }}">{{ $cuadro->name }}<br>
                                    <p style="font-size: 1.6rem; color: white">{{ $cuadro->tamano }}</p>
                                </button>
                                <p style="font-size: 1.7rem">
                                    ${{ number_format((int) str_replace('$', '', $cuadro->precio), 0, ',', '.') }}</p>
                            </div>
                        @endforeach
                ...
                <div class="grupo">
                    ...
                    <div class="botones mt-3 d-flex flex-wrap" style="gap: 4rem;">
                        <div class="row-mb-3 ">
                            <button type="button" class="btn-opcion col-12" data-group="colgante" data-value="Hook"
                                style="max-width: 150px; font-size: 2rem;">Hook</button>
                            <p style="font-size: 1.4rem">Colgante clásico con tornillo</p>
                        </div>

                        <div class="row-mb-3">
                            <button type="button" class="btn-opcion col-12" data-group="colgante" data-value="Iman"
                                style="max-width: 150px; font-size: 2rem;">Imán</button>
        ...
        <div class="d-flex justify-content-center my-4">
            <button id="btnAgregarCarrito" class="tf-button style-1 h50 "><i class="fas fa-shopping-cart"></i>Agregar al
                Carrito</button>
        </div>
        ...
        <div class="mb-5">
            ...
            <p>
                Para que tu cuadro se vea increíble, <strong style="color: #dfdfdf">la calidad de la imagen es
                    clave</strong>. Recomendamos subir archivos en <strong class="resaltado-color">Full HD (1920x1080 px o
                    más)</strong>. Si la imagen tiene baja resolución, contamos con mejoradores automáticos de calidad,
                <strong class="resaltado-color">aunque no todas las fotos pueden optimizarse</strong>.
                <br><br>
                <strong style="color: #d8d3cc">IMPORTANTE:</strong> La orientación del cuadro <strong
                    class="resaltado-color">(horizontal o vertical)</strong> se ajustará según la forma de la
                imagen que nos envíes.
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.getElementById('btnAgregarCarrito').addEventListener('click', async () => {
                ...
                const tamaño = document.querySelector('.btn-opcion.active[data-group="tamaño"]')
                    ?.dataset.value;
                const colgante = document.querySelector('.btn-opcion.active[data-group="colgante"]')
                    ?.dataset.value;
                const file = fileInput.files[0];

                if (!file) return Swal.fire({
                    icon: 'warning',
                    title: 'Por favor, subí una imagen',
                    confirmButtonText: 'Entendido',
                    confirmButtonColor: '#b321a6'
                });
                if (!tamaño) return Swal.fire({
                    icon: 'warning',
                    title: 'Seleccioná un tamaño',
                    confirmButtonText: 'Entendido',
                    confirmButtonColor: '#b321a6'
                });
                if (!colgante) return Swal.fire({
                    icon: 'warning',
                    title: 'Seleccioná un colgante',
                    confirmButtonText: 'Entendido',
                    confirmButtonColor: '#b321a6'
                });
